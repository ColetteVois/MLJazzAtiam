...
def distance_tonnetz(Chord1,
    ...
    root_1, type_1 =  C2V.parse_mir_label(Chord1)
    root_2, type_2 =  C2V.parse_mir_label(Chord2)
    #use Chords2Vec functions to normalize the chords roots
    #(matching the root_list above)
    #root_1 =  C2V.normalized_note(root_1)
    #root_2 =  C2V.normalized_note(root_2)
    ...
    if root_1 == root_2 and type_1 == type_2:
        ...
        a,type_1 =  C2V.parse_mir_label(
                 chordUtil.reduChord(Chord1, 'a0'))
        a,type_2 =  C2V.parse_mir_label(
                 chordUtil.reduChord(Chord2, 'a0'))
...
def distance_euclid(A_1,
    ...
    for each chord. All types of chords will not be supported (see the
     C2V.mir_label_to_semitones_vec function for more information).
    '''
    #initialisation
    A_1_red =  chordUtil.reduChord(A_1, 'reduceWOmodif')
    A_2_red =  chordUtil.reduChord(A_2, 'reduceWOmodif')
    Accord_1_eucl =  C2V.mir_label_to_bin_chroma_vec(A_1_red)
    Accord_2_eucl =  C2V.mir_label_to_bin_chroma_vec(A_2_red)
...
def computeEuclideanPitchVectMat(dictChord):
    matrix = []
    for i in (dictChord):
        matrix.append( C2V.mir_label_to_bin_chroma_vec(i))
    return matrix
...
def save_tonnetz_matrix():
    ...
    inputs0, alphabet, mapping0 =  chordUtil.reduSeq(inputs, 'a0')
...
#%%
a = distance_tonnetz('C:maj', 'C:maj')
print(a)